...
- Download the JAR package from [the download page](https://github.com/streamnative/pulsar-io-bigquery/releases/download/v2.9.3.2/pulsar-io-bigquery-2.9.3.2.jar).
...
2. Build the connector in the `pulsar-io-bigquery` directory.
   ...
   ```bash
   ls target
   pulsar-io-bigquery-2.9.3.2.jar
   ```
   ...
   ```json
    {
        ...
        "archive": "connectors/pulsar-io-bigquery-2.9.3.2.jar",
...
spec:
  image: streamnative/pulsar-io-bigquery:2.9.3.2
  className: org.apache.pulsar.ecosystem.io.bigquery.BigQuerySink
  replicas: 1
  maxReplicas: 1
  input:
  ...
  resources:
    ...
    memory: 1G
  java:
    jar: connectors/pulsar-io-bigquery-2.9.3.2.jar
...
2. Run the Google Cloud BigQuery sink connector.
    ...
    --archive <pulsar-io-bigquery-2.9.3.2.jar>
    ```
